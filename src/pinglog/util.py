...
def parse_reply(text: str, chat_id: int) -> ParsedReply:
    result: ParsedReply = {"entry": text, "xp": 0, "snooze": 0, "silent": False}
    if text:
...
def calculate_xp(entry: str, chat_id: int) -> int:
    total_xp = 0
    ...
    streak_bonus = min(25, get_streak(chat_id)) / 100
    total_xp = int(total_xp * (1 + streak_bonus))
